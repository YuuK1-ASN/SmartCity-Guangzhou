...
这是一个面向广州出行的网页。由于广州市面积较大且天气多变，经常遇到目的地和现在所在地天气状况不一致的情况，但目前主流天气预报app几乎都没有精确查看目的地天气的能力，专业气象网页如windy则不易精确定位目的地地铁站。于是我编写了一个网页，用户可以搜索或点击目的地地铁站，页面会自动找到附近空气监测站，并显示该位置的实时天气、晴雨类别和未来小时预报。方便用户的出行。
...
Windows用户可以直接双击根目录里的 `启动网页.bat`。第一次运行会自动创建虚拟环境、安装依赖并启动网页。
